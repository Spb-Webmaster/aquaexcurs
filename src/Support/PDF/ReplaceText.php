...
class ReplaceText
{
    ...
    public function replaceText($data): bool|null
    {
        ...
        try {
            ...
            /** имя файла */
            $filename = ticketFilePDFName($data['number'], $data['created_at']);



            $originalPdfPath = storage_path('app/public/orders/pdf/templates/input.pdf'); // Исходный PDF
            $modifiedPdfPath = storage_path('app/public/orders/pdf/files/' . $filename); // Результирующий PDF
            ...
            $carbonDateTime = Carbon::now('Europe/Moscow');
