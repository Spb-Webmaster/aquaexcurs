...
class ExcursionEmail extends Model
{
    ...
    {
        $excursionEmail = isset($this->attributes['excursion_id'])
            ? Excursion::find($this->attributes['excursion_id'])
            : null;

        $customEmail = $excursionEmail?->dont_register_form_to_email;

        if($customEmail) {
            $this->attributes['emails'] = $customEmail;
        } else {
